...
- **Per-order live check (user + admin "Check delivery status" buttons):** a single helper resolves the order's `topupghBatchId` → batch and reuses `fetchAndSettleBatchDelivery` (the same idempotent/status-guarded path as the poller), then re-reads persisted state — never trusts client input. User-facing checks MUST pass a cooldown (batch-keyed, ~60s) so end-user clicks can't spend the shared TopUpGH 1-req/min budget the background poller depends on; admins run with no cooldown. On cooldown the helper serves cached `deliveryData` without a provider call. These per-order routes are deliberately NOT in the OpenAPI spec (called via raw fetch from a shared button component), mirroring the batch check button.
