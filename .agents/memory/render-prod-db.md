...
**Pending prod DDL (apply on next Render deploy):** `ALTER TABLE store_orders ADD COLUMN IF NOT EXISTS topupgh_batch_id integer;` — already applied in dev; without it the live app 500s once store orders flow through TopUpGH dispatch.
