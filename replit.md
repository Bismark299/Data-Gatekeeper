...
- **Store order cancel & refund** (`cancelStoreOrderInTx` in `lib/refunds.ts`) — cancelling a PAID store order refunds the full `sellingPrice` to the STORE OWNER's wallet (not a Paystack customer refund; the agent settles with their customer directly). Idempotency anchored on the `refund-store-order-{id}` ledger credit under row lock. Shared by the admin cancel route, the bulk-status `cancelled` path (per-order loop, skips ineligible), and the McBIS poller ("cancelled/canceled" → auto-cancel+refund + delivered='failed'; "failed" → marked failed only). Unpaid orders cancel with no credit; already-cancelled → 400.
